...
def mock_chromadb():
    ...
    with patch("custom_components.pepa_arm_ha.vector_db_manager.chromadb") as mock:
        client = MagicMock()
...
async def test_reindex_indexes_all_entities_bug(
    ...
    with patch("custom_components.pepa_arm_ha.vector_db_manager.CHROMADB_AVAILABLE", True):
        ...
        with patch(
            "custom_components.pepa_arm_ha.vector_db_manager.async_should_expose",
...
async def test_async_setup_registers_state_listeners(mock_hass, mock_chromadb, vector_db_config):
    ...
    with patch("custom_components.pepa_arm_ha.vector_db_manager.CHROMADB_AVAILABLE", True):
        ...
        with (
            patch(
                "custom_components.pepa_arm_ha.vector_db_manager.async_track_time_interval"
            ) as mock_track_time,
...
async def test_embed_with_openai_success(mock_hass, mock_chromadb, vector_db_config):
    ...
    with (
        patch("custom_components.pepa_arm_ha.vector_db_manager.CHROMADB_AVAILABLE", True),
        patch("custom_components.pepa_arm_ha.vector_db_manager.OPENAI_AVAILABLE", True),
    ):
        manager = VectorDBManager(mock_hass, config)
        ...
        with patch("custom_components.pepa_arm_ha.vector_db_manager.openai") as mock_openai:
            ...
            with patch(
                "custom_components.pepa_arm_ha.vector_db_manager.retry_async",
...
async def test_embed_with_openai_library_not_available(mock_hass, mock_chromadb, vector_db_config):
    ...
    with (
        patch("custom_components.pepa_arm_ha.vector_db_manager.CHROMADB_AVAILABLE", True),
        patch("custom_components.pepa_arm_ha.vector_db_manager.OPENAI_AVAILABLE", False),
    ):
        manager = VectorDBManager(mock_hass, config)
    ...
    with patch("custom_components.pepa_arm_ha.vector_db_manager.CHROMADB_AVAILABLE", True):
        ...
        with patch.object(manager, "async_index_entity", AsyncMock()) as mock_index:
            ...
            with (
                ...
                patch(
                    "custom_components.pepa_arm_ha.vector_db_manager.REINDEX_DEBOUNCE_DELAY", 0.05
                ),
            ):
                # Handle the event
...
async def test_debounced_reindex_batches_multiple_entities(
    ...
    with patch("custom_components.pepa_arm_ha.vector_db_manager.CHROMADB_AVAILABLE", True):
        ...
        with (
            ...
            patch("custom_components.pepa_arm_ha.vector_db_manager.REINDEX_DEBOUNCE_DELAY", 0.05),
        ):
